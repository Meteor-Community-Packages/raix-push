...
Push.send = function(options) {
  ...
  if (typeof options.delayUntil !== 'undefined') notification.delayUntil = options.delayUntil;

  if (Match.test(options.apn, Object)) {
    notification.apn = {};
    
    if (Match.test(options.apn.from, String)) notification.apn.from = options.apn.from;
    if (Match.test(options.apn.title, String)) notification.apn.title = options.apn.title;
    if (Match.test(options.apn.text, String)) notification.apn.text = options.apn.text;
    if (Match.test(options.apn.badge, Number)) notification.apn.badge = options.apn.badge;
    if (Match.test(options.apn.sound, String)) notification.apn.sound = options.apn.sound;
    if (Match.test(options.apn.notId, Match.Integer)) notification.apn.notId = options.apn.notId;
  }

  if (Match.test(options.gcm, Object)) {
    notification.gcm = {};
    
    if (Match.test(options.gcm.from, String)) notification.gcm.from = options.gcm.from;
    if (Match.test(options.gcm.title, String)) notification.gcm.title = options.gcm.title;
    if (Match.test(options.gcm.text, String)) notification.gcm.text = options.gcm.text;
    if (Match.test(options.gcm.badge, Number)) notification.gcm.badge = options.gcm.badge;
    if (Match.test(options.gcm.sound, String)) notification.gcm.sound = options.gcm.sound;
    if (Match.test(options.gcm.notId, Match.Integer)) notification.gcm.notId = options.gcm.notId;
  }
