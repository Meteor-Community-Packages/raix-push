        ...
        if (reserved) {
          ...
          serverSend(notification)
            .then(result => {
              ...
              if (result.some(item => item.isFulfilled())) {
                result.forEach(function (item) {
                  if (!item.isFulfilled()) {
                    handleFailedNotification(notification, item.value());
                  }
                });
...
function querySend(query, raixNotification) {
  ...
  Push.appTokens.find(query).forEach(function (appToken) {
    ...
    if (appToken.token.fcm) {
      ...
      firebasePromises.push(Promise.reject({code: 'push/no-fcm-token', message: `ERROR: PUSH.querySend - No fcm token:  ${JSON.stringify(appToken)}`}));
    }
  ...
  if (!notification.failedCount || notification.failedCount <= 5) {
    ...
    Push.notifications.update({_id: notification._id}, {
      $set: {
        ...
        delayUntil: moment().add(2 ** failedCount, 'minutes').toDate()
      }
    });
  }
}
