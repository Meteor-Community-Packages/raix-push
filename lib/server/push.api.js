...
Push.Configure = function(options) {
    ...
    vets.forEach((vet) => {
      ...
      if (connections[key] && connections[key].apn) {
        connections[key].apnConnection = new apn.Connection( connections[key].apn );
        connections[key].apnConnection.on('transmissionError', Meteor.bindEnvironment(function (errCode, notification, recipient) {
          if (Push.debug) {
    ...
    self.sendAPN = function(userToken, notification) {
        ...
        if (!connections[key].apnConnection) {
      ...
      // adds category support for iOS8 custom actions as described here:
        ...
        console.log('PUSH DEBUG: key', key);
        console.log('PUSH DEBUG: connections[key]', connections[key]);
        console.log('PUSH DEBUG: connections[key].apnConnection', connections[key].apnConnection);
        console.log('PUSH DEBUG: note', note);
        console.log('PUSH DEBUG: myDevice', myDevice);

        var result = connections[key].apnConnection.pushNotification(note, myDevice);

        console.log('PUSH DEBUG: called pushNotification(node, myDevice)', result);
    ...
    var initFeedback = function () {
        ...
        for(let key in connections) {
          if (connections[key] && connections[key].apn) {
            var apnOptions = connections[key].apn;
            var feedbackOptions = {
                'batchFeedback': true,
    ...
    self.sendGCM = function(userTokens, notification) {
        ...
        let key = notification.appIdentifier;
        if (!connections[key].gcm || !connections[key].gcm.apiKey) {
        ...
        if (Push.debug) {
          console.log('Create GCM Sender using "' + connections[key].gcm.apiKey + '"');
        }
        var sender = new gcm.Sender(connections[key].gcm.apiKey);
    ...
    if (cfgOptions.sendInterval !== null) {
      ...
      Meteor.setInterval(function() {
          ...
          // var countSent = 0;
          var batchSize = cfgOptions.sendBatchSize || 1;
      ...
      }, cfgOptions.sendInterval || 15000); // Default every 15th sec
    } else {
      if (Push.debug) {
