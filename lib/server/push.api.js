...
function querySend(query, raixNotification) {
  ...
  Push.appTokens.find(query).forEach(function (appToken) {
    ...
    if (Push.debug) {
      console.log('send to token', appToken.fcmToken);
    }

    if (appToken.fcmToken) {
      console.log('appToken.fcmToken', 'passed');
      ...
      firebasePromises.push(Promise.reject({
        codePrefix: 'push',
        errorInfo: {
...
function classifyError(reason) {
  if (reason.codePrefix === 'push') {
    return ERROR_ACTION.EMIT_ERROR;
  } else if (reason.codePrefix === 'messaging') {
    switch (reason.errorInfo) {
