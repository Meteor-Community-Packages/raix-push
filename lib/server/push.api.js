...
Push.Configure = function(options) {
    ...
    const vets = Vets.find({$or: [{disabled: {$exists: false}}, {disabled: false}]}).fetch();
    vets.forEach((vet) => {
    ...
    if (cfgOptions.sendInterval !== null && !(process.env.PUSH_NOTIFICATIONS_DISABLED === true || process.env.PUSH_NOTIFICATIONS_DISABLED === 'true')) {
      ...
      Meteor.setInterval(function() {
          ...
          const pendingNotifications = Push.notifications.find({ $and: [
                // Message is not sent
                { sent : { $ne: true } },
          ...
          pendingNotifications.forEach(function(notification) {
              ...
              const reserved = Push.notifications.update({ $and: [
              ...
              if (reserved) {
                ...
                try {
                  ...
                  console.log('Exception sending notification ' + JSON.stringify(e));
                }
