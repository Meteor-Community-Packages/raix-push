...
Push.Configure = function(options) {
  ...
  if (options.apn) {
    ...
    var apnProvider = new apn.Provider(options.apn);
    ...
    self.sendAPN = function(userToken, notification) {
      ...
      if (typeof notification.title !== 'undefined') {
        note.aps.alert.title = notification.title;
      }
  ...
  if (options.gcm && options.gcm.apiKey) {
    ...
    self.sendGCM = function(userTokens, notification) {
      ...
      if (typeof notification.android_channel_id !== 'undefined') {
        data.android_channel_id = notification.android_channel_id;
      } else if (Push.debug) {
        console.warn(`For devices running Android 8.0 or later you are required to provide an android_channel_id. See https://github.com/raix/push/issues/341 for more info`);
      }
