...
const firebaseAdmin = Npm.require('firebase-admin');

Push.setBadge = function (/* id, count */) {
  // throw new Error('Push.setBadge not implemented on the server');
};

// TODO @slobodan Maybe move this to server.js file
Push.Connections = {};
...
Push.Configure = function (options) {
  ...
  const initFeedback = function () {
    ...
    Object.keys(Push.Connections).forEach(key => {
      if (Push.Connections[key] && Push.Connections[key].apn) {
        const apnOptions = Push.Connections[key].apn;
        ...
        feedback.on('feedback', function (devices) {
          devices.forEach(function (item) {
            ...
            // TODO UNCOMMENT THIS - TEMPORARY DISABLED FOR TROUBLESHOOTING
            //_removeToken({
            //    apn: item.device
            //});
          });
  ...
  self.fcmSingleFCM = function (userFcmToken, raixNotification) {
    ...
    let appIdentifier = raixNotification.appIdentifier;
  ...
  self.sendGCM = function (userTokens, notification) {
    ...
    const sender = new gcm.Sender(Push.Connections[key].gcm.apiKey);
  ...
  const _querySend = function (query, raixNotification) {
    const countApn = [];
    const countGcm = [];
    ...
    Push.appTokens.find(query).forEach(function (appToken) {
      ...
      if (appToken.token.fcm) {
        ...
        self.fcmSingleFCM(appToken.token.fcm, raixNotification);
      } else {
  ...
  const _topicsSend = function (topics, raixNotification) {
    // TODO @slobodan Implement fcm topic send
  };

  self.serverSend = function (raixNotification) {
    ...
    if (query) {
      ...
      return _querySend(query, raixNotification);
    } else if (raixNotification.topics) {
      return _topicsSend(raixNotification.topics, raixNotification);
    } else {
    ...
    Meteor.setInterval(function () {
      ...
      pendingNotifications.forEach(function (notification) {
        ...
        if (reserved) {
          ...
          let result;
          try {
            result = Push.serverSend(notification);
          } catch (e) {
