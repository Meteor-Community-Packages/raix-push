...
function convertApnToFcmToken(appName, tokenObject) {
  ...
  HTTP.post('https://iid.googleapis.com/iid/v1:batchImport', {
    ...
    data: {
      'application': getIosBundleId(vet),
      'sandbox': false,
...
function getIosBundleId(vet) {
  if (vet.appIdentifier === 'faulkvilleanimalhospital') {
    return `com.myvet2pet.${vet.appIdentifier}`;
  }
  return vet.iosLegacyAppIdentifier || `com.vet2pet.${vet.appIdentifier}`;
}
