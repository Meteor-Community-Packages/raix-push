...
Push.addListener('token', function (currentToken, value) {
  if (value) {
    ...
    Push.appTokens.update({token: currentToken}, {$set: {token: value}}, {multi: true});
  } else if (value === null) {
    // Remove the token for app
    Push.appTokens.update({token: currentToken}, {$unset: {token: true}}, {multi: true});
  }
});
...
Meteor.methods({
  'raix:push-update': function (options) {
    ...
    if (doc) {
      ...
      const removed = Push.appTokens.remove({
        $and: [
          {_id: {$ne: doc._id}},
  ...
  'raix:configure-push-connections': (vetId) => {
    ...
    if (process.env.NODE_ENV === 'production') {
      Push.FirebaseApps[key] = firebaseAdmin.initializeApp(
        {
