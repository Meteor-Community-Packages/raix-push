...
function getConfigForApp(app, resolve, reject) {
  app.getConfig()
    .then(response => {
      resolve(response);
    })
    ...
    listApps(projectManagement, platform).then(apps => {
      ...
      if (!apps || apps.length === 0) {
        createApp(projectManagement, vet, platform)
          .then(app => {
            if (!app) {
              reject(new Error(`Error creating android app for ${vetId}`));
            }

            getConfigForApp(app, resolve, reject);
          })
          .catch(error => {
            reject(error);
          });
      } else if (apps.length === 1) {
        getConfigForApp(apps[0], resolve, reject);
      } else {
        console.log('PUSH: Multiple android apps found', apps);
