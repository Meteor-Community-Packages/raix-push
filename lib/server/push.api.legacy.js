...
Push.legacy.Configure = function (options) {
  ...
  const _querySend = function (query, options) {
    ...
    let vetIdentifiersSet = new Set();

    let appTokens = Push.appTokens.find(query);
    ...
    appTokens.forEach(function (appToken) {
      ...
      vetIdentifiersSet.add(appToken.appName);
    });

    let users = Meteor.users.find({_id: {$in: userIdsSet}}).fetch();
    let vets = Vets.find({appIdentifier: {$in: vetIdentifiersSet}}).fetch();
    ...
    appTokens.forEach(function (app) {
      ...
      if (app.userId) {
        if (!userVetActiveMap.get(app.userId + app.appName)) {
          return;
        }
      }
